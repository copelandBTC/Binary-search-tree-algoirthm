...
package body bin_tree is
   ...
   procedure free is new ada.unchecked_deallocation (node, tree_pnt);
   ...
   begin
      ...
      -- Blank phone number implies itme not found
      if q.info.phone = "          " then
         return;
      end if;
      ...
      new_line;
      put (name); put (" deleted");
      new_line;
   end delete;

   function find_iterative (name : string10) return tree_pnt is
      p, blank : tree_pnt;
      cust : customer;
   begin
   ...
   function find_recursive (name : string10) return tree_pnt is
      p : tree_pnt;
      cust : customer;
   begin
      cust.name := name;
   ...
   procedure preorder (point : in tree_pnt) is
      p : tree_pnt;
      package stk is new stack (20, tree_pnt);
   begin
      p := point;
   ...
   procedure postorder_iter (point : in tree_pnt) is
      p : tree_pnt;
   ...
   procedure postorder_recurs (point : in tree_pnt) is
      p : tree_pnt;
      rec : stk_rec;
      d   : natural range 0 .. 1;
   ...
   j : integer := 1;
   procedure reverse_in (point : tree_pnt) is
      q : tree_pnt;
   begin
      q := point;
   ...
   function inorder_succ (point : in tree_pnt) return tree_pnt is
      q : tree_pnt;
   begin
      q := point.rlink;
   ...
   function inorder_pred (point : in tree_pnt) return tree_pnt is
      q : tree_pnt;
   begin
      q := point.llink;
   ...
   function cust_name (point : in tree_pnt) return string10 is
   begin
      return point.info.name;
   end cust_name;

   function cust_phone (point : in tree_pnt) return string10 is
   begin
      return point.info.phone;
   end cust_phone;

   function root return tree_pnt is
   begin
      return head.llink;
   end root;

   procedure print_info (point : tree_pnt) is
   begin
      new_line;
   ...
   begin
      ...
      new_line;
      put_line ("Deallocating all nodes from tree...");
      new_line;
      ...
      new_line;
      put_line ("All nodes of the tree have been deallocated");
      new_line;
   end free_all;
   begin
      initialize;
